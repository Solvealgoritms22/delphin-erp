      ...
      <div class="flex flex-col sm:flex-row flex-0 sm:items-center sm:justify-between p-6 sm:pb-8 sm:pt-10 border-b bg-card dark:bg-transparent">
        <div class="flex-1 min-w-0">
          <div class="flex items-center cursor-pointer mb-2" (click)="goBack()">
            ...
            <span class="text-sm font-medium text-secondary">{{ 'commercial.clients.back' | transloco }}</span>
             </div>

           <h2 class="text-3xl md:text-4xl font-extrabold tracking-tight leading-7 sm:leading-10 truncate">
             {{ (isEdit ? 'commercial.clients.edit' : 'commercial.clients.new') | transloco }}
          </h2>
        </div>
      </div>
      ...
      <form [formGroup]="form" class="flex w-full flex-col md:flex-row flex-auto p-6 sm:p-10 gap-6">
        ...
        <div class="flex flex-col w-full md:w-2/3 gap-6">
          ...
          <div class="flex flex-col bg-card rounded-2xl shadow-sm border p-6 sm:p-8">
           ...
           <div class="flex flex-col bg-card rounded-2xl shadow-sm border p-6 sm:p-8">
             <h2 class="text-lg font-bold mb-1">{{ 'commercial.form.clientLogo' | transloco }}</h2>
...
export class ClientForm implements OnInit {
  ...
  ngOnInit() {
    ...
    this.isEdit = !!this.clientId && this.clientId !== 'new';
    ...
    if (this.isEdit && this.clientId) {
      this.clientsService.findOne(this.clientId).subscribe(data => {
  ...
  onLogoSelected(event: Event): void {
    ...
    reader.onload = () => { this.logoPreview = reader.result as string; this.form.patchValue({ logo: this.logoPreview }); };
    reader.readAsDataURL(file);
  }

  goBack() {
    this.router.navigate(['/admin/commercial/clients']);
  }
  ...
  submit() {
    ...
    if (this.isEdit) {
      this.clientsService.update(this.clientId!, value).subscribe(() => this.goBack());
    } else {
      this.clientsService.create(value).subscribe(() => this.goBack());
    }
  }
}
