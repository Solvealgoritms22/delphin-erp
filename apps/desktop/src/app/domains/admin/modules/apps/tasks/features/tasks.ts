        ...
        >
          ...
          @if (query.isPending()) {
            <div class="p-8 flex flex-col gap-4">
              <app-skeleton type="rect" height="3rem" />
              <app-skeleton type="rect" height="3rem" />
              <app-skeleton type="rect" height="3rem" />
            </div>
          } @else if (query.data()?.tasks && query.data()!.tasks.length > 0) {
            <div
              class="divide-y"
              cdkDropList
              [cdkDropListData]="query.data()!.tasks"
              (cdkDropListDropped)="orderTasks($event)"
            >
              <!-- Task -->
              @for (task of query.data()!.tasks; track task.id; let first = $first) {
                <div
                  [id]="'task-' + task.id"
