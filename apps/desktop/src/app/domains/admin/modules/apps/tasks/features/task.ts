...
import { Task as TaskModel } from '@/app/domains/admin/modules/apps/tasks/data/model';
...
export default class Task {
  ...
  // State
  protected query = this.tasksService.tasksQuery;
  protected task = computed(() => {
    const data = this.query.data();
    if (!data) return {} as TaskModel;
    return data.tasks.find((t) => t.id === this.taskId()) ?? ({} as TaskModel);
  });
  protected tags = computed(() => this.query.data()?.tags ?? []);
  protected taskModel = linkedSignal<TaskModel>(() => this.task() ?? ({} as TaskModel));
