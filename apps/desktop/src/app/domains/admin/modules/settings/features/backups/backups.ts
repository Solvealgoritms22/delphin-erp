...
@Component({
  ...
  imports: [DatePipe, MatButtonModule, MatIconModule, MatSnackBarModule, TranslocoPipe, EmptyStateComponent],
  template: `
    <div class="flex h-full w-full min-w-0 flex-col bg-white dark:bg-neutral-900">
      <header class="flex shrink-0 flex-col justify-between gap-6 border-b border-neutral-200 px-6 py-8 sm:flex-row sm:items-center md:px-8 dark:border-neutral-700">
        ...
        <div class="flex flex-wrap gap-2">
          <button mat-stroked-button type="button" (click)="connectDrive()"><mat-icon svgIcon="cloud" class="mr-2"></mat-icon>{{ 'backups.connectDrive' | transloco }}</button>
...
export class BackupsComponent {
  ...
  remove(backup: Backup) { if (!window.confirm(this.i18n.translate('backups.confirmDelete'))) return; this.http.delete(`${this.api}/${backup.id}`).subscribe({ next: () => this.load(), error: () => this.notice('backups.deleteError') }); }
