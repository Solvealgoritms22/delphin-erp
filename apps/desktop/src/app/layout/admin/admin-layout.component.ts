      ...
      <mat-sidenav-content class="flex flex-col h-full min-h-0 overflow-hidden">
        ...
        >
          ...
          <div class="flex items-center gap-x-2" style="-webkit-app-region: no-drag">
            ...
            <!-- Animated Assistant Drawer Trigger (Hides smoothly on AI Chat module) -->
            <div
              class="flex items-center overflow-hidden transition-all duration-300 ease-out origin-right"
              [class.max-w-16]="!isAiChatRoute()"
              [class.opacity-100]="!isAiChatRoute()"
              [class.scale-100]="!isAiChatRoute()"
              [class.max-w-0]="isAiChatRoute()"
              [class.opacity-0]="isAiChatRoute()"
              [class.scale-75]="isAiChatRoute()"
              [class.pointer-events-none]="isAiChatRoute()"
            >
              <mat-divider
                vertical
...
export class AdminLayout implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    // Monitor current route to smoothly animate header assistant button
    this.isAiChatRoute.set(this.checkIfAiChatRoute(this.router.url));
    this.router.events
      .pipe(
      ...
      .subscribe((e) => {
        this.isAiChatRoute.set(this.checkIfAiChatRoute(e.urlAfterRedirects || e.url));
      });
  ...
  private checkIfAiChatRoute(url: string): boolean {
    return url.includes('/admin/ai-chat') || url.includes('/ai-chat');
  }
