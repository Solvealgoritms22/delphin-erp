...
export class ReportsController {
  ...
  getTopProducts(
    ...
    @Query() query: TopProductsReportDto,
  ) {
  ...
  @RequirePermissions('reports:read')
  @ApiOperation({
    summary: 'Reporte de cuentas por cobrar y antigüedad de saldos',
  })
  ...
  @RequirePermissions('reports:read')
  @ApiOperation({
    summary: 'Reporte de valoración de inventario y alertas de stock bajo',
  })
  ...
  getSalesByClient(
    @CurrentUser() user: any,
    @Query() query: DateRangeReportDto,
  ) {
  ...
  get606(@CurrentUser() user: any, @Query() query: TaxReportDto) {
    return this.reportsService.get606Report(
      user.empresaId,
      query.periodo,
      query.sucursalId,
    );
  ...
  get607(@CurrentUser() user: any, @Query() query: TaxReportDto) {
    return this.reportsService.get607Report(
      user.empresaId,
      query.periodo,
      query.sucursalId,
    );
  ...
  get608(@CurrentUser() user: any, @Query() query: TaxReportDto) {
    return this.reportsService.get608Report(
      user.empresaId,
      query.periodo,
      query.sucursalId,
    );
  ...
  getIt1(@CurrentUser() user: any, @Query() query: TaxReportDto) {
    return this.reportsService.getIt1Report(
      user.empresaId,
      query.periodo,
      query.sucursalId,
    );
  ...
  @RequirePermissions('reports:read')
  @ApiOperation({
    summary: 'Descargar archivo plano TXT para la DGII (606, 607, 608)',
  })
