  ...
  Object.fromEntries(
    NOTIFICATION_CATALOG.map((event) => [
      notificationEmailKey(event.id),
      {
        ...
        footer:
          'Recibes este aviso según tus preferencias de notificaciones en Dolphin ERP.',
        accent:
...
export const EMAIL_CATALOG: Record<string, EmailDefinition> = {
  ...
  notification: {
    ...
    subject: '{{title}}',
    heading: '{{title}}',
    body: 'Tienes una nueva actualización en Dolphin ERP.',
    footer: 'Este es un aviso automático de Dolphin ERP.',
    accent: '#2563eb',
