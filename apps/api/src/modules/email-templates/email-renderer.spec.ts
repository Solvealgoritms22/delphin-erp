...
describe('email renderer', () => {
  it('escapes every recipient-controlled value in the HTML document', () => {
    const email = renderEmail(
      ...
      undefined,
      { rows: [{ label: '<b>Item</b>', value: '<i>10</i>' }] },
    );
  ...
  it('keeps the activation destination server controlled and HTTPS', () => {
    ...
    expect(email.html).toContain(
      'https://erp.example/auth/accept-invitation?token=opaque',
    );
  ...
  it('requires six-digit OTP codes and adds security guidance', () => {
    expect(() =>
      renderEmail('verification', { name: 'Ana' }, undefined, { code: 'abc' }),
    ).toThrow(BadRequestException);
    const email = renderEmail('verification', { name: 'Ana' }, undefined, {
      code: '123456',
    });
  ...
  it('renders rich HTML body with images while keeping variables safely escaped', () => {
    ...
    expect(email.html).toContain(
      '<img src="https://cdn.example.com/logo.png" alt="Logo" style="max-width:100%;height:auto" />',
    );
    expect(email.html).toContain(
  ...
  it('only includes dolphin logo in system templates, not in tenant templates', () => {
    ...
    expect(quoteEmail.attachments.some((a) => a.cid === 'dolphin-brand')).toBe(
      false,
    );
    expect(quoteEmail.html).toContain('cid:dolphin-signature');
    expect(
      quoteEmail.attachments.some((a) => a.cid === 'dolphin-signature'),
    ).toBe(true);
    ...
    expect(quoteWithLogo.html).toContain(
      'https://cdn.example.com/farmacia-logo.png',
    );
    ...
    expect(
      verificationEmail.attachments.some((a) => a.cid === 'dolphin-brand'),
    ).toBe(true);
