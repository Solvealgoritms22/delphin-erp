...
export function renderEmail(key: EmailKey, values: Record<string,string>, custom?: EmailDesign | null, blocks: EmailBlocks = {}) {
  ...
  const fill = (value:string) => value.replace(/{{\s*([^{}]+?)\s*}}/g, (_,name:string) => {
  ...
  const heading = fill(design.heading), body = fill(design.body), footer = fill(design.footer);
