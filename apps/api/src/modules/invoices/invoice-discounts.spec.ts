...
function line(base: number, rate: number): DiscountLine {
  return {
    cantidad: d(1),
    precioUnitario: d(base),
    descuento: d(0),
    porcentajeDescuento: d(0),
    subtotal: d(base),
    tasaItbis: d(rate),
    itbis: d(base).mul(rate).div(100),
    total: d(base).mul(1 + rate / 100),
  };
}
describe('global discounts', () => {
  it('reduces each tax base proportionally, including exempt lines', () => {
    ...
    expect(lines.map((x) => x.subtotal.toFixed(2))).toEqual([
      '90.00',
      '90.00',
      '90.00',
    ]);
    expect(lines.map((x) => x.itbis.toFixed(2))).toEqual([
      '16.20',
      '14.40',
      '0.00',
    ]);
  ...
  it('allocates a one-cent residual exactly once', () => {
    ...
    expect(lines.reduce((s, x) => s.add(x.descuento), d(0)).toFixed(2)).toBe(
      '0.01',
    );
  ...
  it.each([-1, 101])('rejects out-of-range discount %s', (amount) => {
    expect(() =>
      allocateGlobalDiscount([line(100, 18)], d(amount), 2),
    ).toThrow();
  });
});
