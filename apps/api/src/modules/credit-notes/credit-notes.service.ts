...
export class CreditNotesService {
  ...
  async create(empresaId: string, usuarioId: string, dto: CreateCreditNoteDto) {
    ...
    for (const line of dto.lines) {
      ...
      const current =
        lineQuantityMap.get(line.detalleOriginalId) || new Prisma.Decimal(0);
    ...
    const { creditNote, needsFiscal, totalCredit, original } =
      await this.prisma.$transaction(
        ...
        `;
          ...
          const isElectronic = originalInv.tipoNcf
            ?.toUpperCase()
            .startsWith('E');
          ...
          for (const [
            detalleOriginalId,
            requestedQty,
          ] of lineQuantityMap.entries()) {
            const detalle = originalInv.detalles.find(
              (d) => d.id === detalleOriginalId,
            );
            if (!detalle) {
              throw new BadRequestException(
            ...
            creditedLines.push({
              ...
              indicadorFacturacion:
                ...
                (detalle.tasaItbis.eq(0)
                  ? '4'
                  : detalle.tasaItbis.eq(16)
                    ? '2'
                    : '1'),
