...
export class MfaService {
  ...
  async status(userId: string) {
    const row = await this.prisma.mfaCredential.findUnique({
    ...
    if (!row?.enabledAt) {
      if (user.mfaHabilitado)
        throw new UnauthorizedException(
          'La cuenta requiere recuperación del segundo factor',
        );
      return null;
    }
  ...
  async setup(user: any) {
    if (user.authTime === 1)
      throw new BadRequestException(
        'Vuelve a iniciar sesión antes de configurar 2FA',
      );
    const secret = new Secret({ size: 20 }).base32;
    const encrypted = encryptSecret(secret);
    await this.prisma.$transaction(async (tx) => {
      await tx.$queryRaw`SELECT id FROM usuarios WHERE id = ${user.id} FOR UPDATE`;
      const row = await tx.mfaCredential.findUnique({
        where: { usuarioId: user.id },
      });
      if (row?.enabledAt)
      ...
      await tx.mfaCredential.upsert({
        ...
        create: {
          usuarioId: user.id,
          secret: encrypted,
          setupExpiresAt: new Date(Date.now() + 600_000),
        },
        update: {
          secret: encrypted,
          setupExpiresAt: new Date(Date.now() + 600_000),
    ...
    return {
      secret,
      qrDataUrl: await QRCode.toDataURL(uri, { errorCorrectionLevel: 'M' }),
    };
  }
  ...
  private async check(tx: any, row: any, code: string): Promise<boolean> {
    ...
    const recovered =
      row.enabledAt && /^[a-f0-9]{32}$/.test(normalized)
        ? row.recoveryHashes.find((saved: string) =>
            equal(saved, hash(normalized)),
          )
        : undefined;
    const delta = /^\d{6}$/.test(normalized)
      ? this.totp(decryptSecret(row.secret)).validate({
          token: normalized,
          window: 1,
          timestamp: now,
        })
    ...
    if (!recovered && (delta === null || counter <= row.lastCounter)) {
      ...
      await tx.mfaCredential.update({
        ...
        data: {
          ...
          lockedUntil: attempts >= 5 ? new Date(now + 900_000) : null,
        },
      });
      return false;
    }
  ...
  async complete(token: string, code: string, request?: any) {
    ...
    if (!userId)
      throw new UnauthorizedException(
        'Código inválido, reutilizado o desafío expirado',
      );
    return userId;
  }
  ...
  async manage(
    ...
    const success = await this.prisma.$transaction(async (tx) => {
      ...
      if (action === 'disable')
        await tx.mfaCredential.delete({ where: { usuarioId: user.id } });
      else
        await tx.mfaCredential.update({
          where: { usuarioId: user.id },
          data: {
            ...
            setupExpiresAt: null,
            recoveryHashes: codes.map(hash),
          },
        });
      await tx.usuario.update({
        where: { id: user.id },
        data: { mfaHabilitado: action !== 'disable' },
      });
      await tx.authFlow.updateMany({
        where: { nonce: user.id, status: 'MFA_LOGIN' },
        data: { status: 'CONSUMED' },
      });
    ...
    if (!success)
      throw new BadRequestException(
        'Código inválido, reutilizado o configuración expirada',
      );
