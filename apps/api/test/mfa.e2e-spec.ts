...
if (testUrl && !/^dolphin_test_/.test(new URL(testUrl).pathname.slice(1))) {
  throw new Error(
    'MFA integration tests require an isolated dolphin_test_ database',
  );
}
...
suite('MFA persistence and concurrency (PostgreSQL)', () => {
  ...
  beforeAll(async () => {
    process.env.SECRETS_ENCRYPTION_KEYS = JSON.stringify({
  ...
  beforeEach(async () => {
    ...
    const enabled = await service.manage(
      user,
      otp.generate({ timestamp: Date.now() - 30_000 }),
      'enable',
    );
  ...
  it('encrypts the secret and stores only recovery hashes', async () => {
    const row = await db.mfaCredential.findUniqueOrThrow({
  ...
  it('accepts only one concurrent verification of a TOTP and challenge', async () => {
    ...
    expect(await db.userSession.count({ where: { usuarioId: user.id } })).toBe(
      0,
    );
  });

  it('consumes a recovery code across different challenges', async () => {
    const a = await service.challenge(user),
      b = await service.challenge(user);
    const results = await Promise.allSettled([
      service.complete(a!.challengeToken, codes[0]),
      service.complete(b!.challengeToken, codes[0]),
    ]);
    expect(
      results.filter((result) => result.status === 'fulfilled'),
    ).toHaveLength(1);
  ...
  it('persists the account lock even when the request fails', async () => {
    ...
    for (let i = 0; i < 5; i++)
      await expect(
        service.complete(challenge!.challengeToken, 'bad'),
      ).rejects.toThrow();
    await expect(
      service.complete(challenge!.challengeToken, codes[0]),
    ).rejects.toThrow();
    const row = await db.mfaCredential.findUniqueOrThrow({
      where: { usuarioId: user.id },
    });
  ...
  it('revokes other sessions when disabling and leaves an audit record', async () => {
    await db.userSession.create({
      data: { usuarioId: user.id, tokenHash: 'synthetic-' + randomUUID() },
    });
    ...
    expect(
      await db.activityLog.count({
        where: { usuarioId: user.id, accion: 'MFA_DISABLE' },
      }),
    ).toBe(1);
  });

  it('rejects enrollment from an old session', async () => {
    await expect(service.setup({ ...user, authTime: 1 })).rejects.toThrow(
